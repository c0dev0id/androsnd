...
Testability is uneven, and the seams matter: `PlaylistManager`'s library can only be populated by a real SAF tree scan, so everything downstream of `scanFolder` — the cursor **write** path (`moveCursorTo`), `selectNextQueueSong`, the display-order index rebuild — is not reachable from a unit test. Three members are `internal` purely so their decisions can be tested against plain data: `indexOfRememberedSong`, `isAudioFile`, `isCoverImage`. That is a keyhole, not a design: the real fix is to separate walking the SAF tree from deciding what to do with the result, at which point those seams close and the write path becomes reachable. Prefer that over building a fake `DocumentsProvider`.
